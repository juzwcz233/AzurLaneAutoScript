...
class GGHandler:
    ...
    def restart(self, crashed=False):
        ...
        for _ in range(2):
            try:
                ...
                if not timeout(LoginHandler(config=self.config, device=self.device).app_restart, timeout_sec=1200):
                    break
                raise RuntimeError
    ...
    def set(self, mode=True):
        ...
        if mode:
            ...
            success = timeout(
                GGU2(config=self.config, device=self.device).set_on,
    ...
    def check_status(self, mode=True):
        ...
        if gg_data['gg_enable']:
            gg_on = mode if deep_get(d=self.config.data,
