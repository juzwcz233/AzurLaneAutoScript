...
class GGScreenshot(Base):
    ...
    def skip_error(self):
        ...
        while 1:
            ...
            if timeout.reached():
                break
            if self.appear_then_click(button=BUTTON_GG_RESTART_ERROR, offset=(50, 50), interval=2):
        ...
        while 1:
            ...
            if self.appear_then_click(button=BUTTON_GG_RESTART_ERROR, offset=(50, 50), interval=2):
                ...
                skipped = 1
                continue
            if self.appear_then_click(button=BUTTON_GG_STOP, offset=(50, 50), interval=2):
                logger.info('Stop lua')
                skipped = 1
                continue
            if self.appear_then_click(button=BUTTON_GG_SCRIPT_END, offset=(50, 50), interval=2):
                logger.info('Close previous script')
                skipped = 1
                continue
            if self.appear_then_click(button=BUTTON_GG_SCRIPT_FATAL, offset=(50, 50), interval=2):
                logger.info('Restart previous script')
                skipped = 1
                continue
            if self.appear_then_click(button=BUTTON_GG_APP_CHOOSE0, offset=(50, 50), interval=2):
                logger.info('APP choose')
                skipped = 1
                continue
            if self.appear_then_click(button=BUTTON_GG_APP_CHOOSE1, offset=(50, 50), interval=2):
                logger.info('APP Choose')
                continue
            if self.appear(button=BUTTON_GG_SCRIPT_MENU_A, offset=(50, 50), interval=2):
                logger.info('Close previous script')
                self.device.click(BUTTON_GG_EXIT_POS)
                skipped = 1
                continue
            if self.appear(button=BUTTON_GG_SEARCH_MODE_CONFIRM, offset=(10, 10), interval=2, threshold=0.5):
                logger.info('At GG main panel, click GG exit')
                self.device.click(BUTTON_GG_EXIT_POS)
                skipped = 1
                continue
            if self.appear(button=BUTTON_GG_CONFIRM, offset=(50, 50), interval=2) and not self.appear(button=BUTTON_GG_CONFIRM, offset=(10, 10)):
            ...
            if self.appear(button=BUTTON_GG_CONFIRM, offset=(10, 10)):
                ...
                skipped = 1
                continue
            if not self.appear(button=BUTTON_GG_CONFIRM, offset=(50, 50), interval=2):
    ...
    def enter_gg(self):
        ...
        while 1:
            ...
            for i in range(len(self.method)):
                if self.appear(button=self.method[int(i)], offset=(50, 50), interval=2):
                    self.device.click(BUTTON_GG_ENTER_POS)
                    break
            if self.appear(button=BUTTON_GG_ENTER, offset=(50, 50), interval=2) or self.appear(button=BUTTON_GG_CONFIRM, offset=(50, 50), interval=2):
                logger.hr('Enter GG')
        ...
        while 1:
            ...
            if self.appear_then_click(button=BUTTON_GG_APP_CHOOSE1, offset=(50, 50), interval=2):
                logger.info('APP Choose')
                continue
            if self.appear(button=BUTTON_GG_APP_ENTER, offset=(50, 50), interval=2, threshold=0.999):
                logger.info('APP Enter')
                break
            if not self.appear(button=BUTTON_GG_APP_ENTER, offset=(50, 50), interval=2, threshold=0.999):
    ...
    def gg_enter_script(self):
        ...
        while 1:
            self.device.screenshot()
            if self.appear(button=BUTTON_GG_STOP, offset=(50, 50), interval=2):
                self.config.task_call('Restart')
                self.config.task_stop()
            if self.appear(button=BUTTON_GG_SCRIPT_ENTER_CONFIRM, offset=(50, 50), interval=2):
                self.gg_lua()
            ...
            if self.appear(button=BUTTON_GG_SEARCH_MODE_CONFIRM, offset=(10, 10), interval=2, threshold=0.5):
                ...
                continue
            if not self.appear(button=BUTTON_GG_SEARCH_MODE_BUTTON, offset=(50, 50), interval=2):
                self.device.click(BUTTON_GG_TAB_SEARCH_POS)
                logger.info('Enter search mode')
                continue
        ...
        while 1:
            if skip_first_screenshot:
                ...
                self.device.screenshot()
            if self.appear_then_click(button=BUTTON_GG_SCRIPT_START, offset=(50, 50), interval=2):
                continue
            if self.appear_then_click(button=BUTTON_GG_STOP, offset=(50, 50), interval=2):
                continue
            if self.appear(button=BUTTON_GG_SCRIPT_MENU_A, offset=(50, 50), interval=2):
    ...
    def gg_mode(self):
        ...
        while 1:
            ...
            if self.appear(button=BUTTON_GG_STOP, offset=(50, 50), interval=2):
                self.config.task_call('Restart')
                self.config.task_stop()
            if self.appear_then_click(button=BUTTON_GG_SCRIPT_MENU_A, offset=(50, 50), interval=2):
                continue
            if self.appear(button=BUTTON_GG_SCRIPT_START_PROCESS, offset=(50, 50), interval=2):
                break
    ...
    def gg_handle_factor(self):
        ...
        if (isinstance(self.factor, int) == True or isinstance(self.factor, float) == True) and (1 <= self.factor <= 1000):
            ...
            for i in str(self.factor):
                self.appear_then_click(button=method[int(i)], offset=(50, 50), interval=2)
    ...
    def gg_script_run(self):
        ...
        while 1:
            if skip_first_screenshot:
                ...
                self.device.screenshot()
            if self.appear_then_click(button=BUTTON_GG_SCRIPT_START_PROCESS, offset=(50, 50), interval=2):
                continue
            if self.appear(button=BUTTON_GG_SCRIPT_START_CHECK, offset=(50, 50), interval=2):
                break
        ...
        while 1:
            if skip_first_screenshot:
                ...
                self.device.screenshot()
            for i in range(len(self.method)):
                if self.appear(button=self.method[int(i)], offset=(50, 50), interval=2, threshold=0.999):
                    self.device.click(BUTTON_GG_ENTER_POS)
                    break
            if self.appear_then_click(button=BUTTON_GG_SCRIPT_END, offset=(50, 50), interval=2):
                count += 1
                continue
            if self.appear(button=BUTTON_GG_SEARCH_MODE_BUTTON, offset=(50, 50), interval=2) and count != 0:
                return 1
        ...
        if (self.gg_action == 'auto' and self.gg_package_name != 'com.') or (self.gg_action == 'manual' and self.gg_package_name != 'com.'):
            ...
            while 1:
                ...
                if self.appear(button=BUTTON_GG_STOP, offset=(50, 50), interval=2):
                    self.config.task_call('Restart')
                    self.config.task_stop()
                if self.appear_then_click(button=BUTTON_GG_SKIP0, offset=(50, 50), interval=2):
                    count += 1
                    continue
                if self.appear_then_click(button=BUTTON_GG_SKIP1, offset=(50, 50), interval=2):
                    count += 1
                    continue
                if self.appear(button=BUTTON_GG_ENTER, offset=(50, 50), interval=2):
                    self.device.click(BUTTON_GG_EXIT_POS)
                    count += 1
                    continue
                if self.appear(button=BUTTON_GG_CONFIRM, offset=(50, 50), interval=2):
                    self.device.click(BUTTON_GG_EXIT_POS)
                    count += 1
                    continue
                if self.appear_then_click(button=BUTTON_GG_START, offset=(50, 50), interval=2):
                    count += 1
                    continue
                for i in range(len(self.method)):
                    if self.appear(button=self.method[int(i)], offset=(50, 50), interval=2) and count != 0:
                        return 1
    ...
    def gg_lua(self):
        ...
        if self.gg_action == 'auto' and self.gg_package_name != 'com.':
            while 1:
                self.device.screenshot()
                if not self.appear(button=OCR_GG_LUAPATH, offset=(50, 50), interval=2) and self.appear(button=BUTTON_GG_LUACHOOSE, offset=(50, 50), interval=2):
                    break
                if self.appear(button=OCR_GG_LUAPATH, offset=(50, 50), interval=2):
                    return 1
            ...
            while 1:
                ...
                if self.appear(button=BUTTON_GG_SCRIPT_START_PROCESS, offset=(50, 50), interval=2):
                    self.device.click(BUTTON_GG_LUACHOOSE)
                    continue
                if not self.appear(button=BUTTON_GG_ENTER_PATH0, offset=(50, 50), interval=2) and self.appear(button=BUTTON_GG_BACK, offset=(50, 50), interval=2):
                    self.device.click(BUTTON_GG_BACK)
                    continue
                if self.appear(button=BUTTON_GG_ENTER_PATH0, offset=(50, 50), interval=2):
                    self.device.click(BUTTON_GG_BACK)
                    continue
                if self.appear(button=BUTTON_GG_ENTER_PATH1, offset=(50, 50), interval=2):
                    break
            ...
            while 1:
                if skip_first_screenshot:
                    ...
                    self.device.screenshot()
                if self.appear_then_click(button=BUTTON_GG_PATH0, offset=(50, 50), interval=2):
                    continue
                if self.appear_then_click(button=BUTTON_GG_PATH1, offset=(50, 50), interval=2):
                    continue
                if self.appear_then_click(button=BUTTON_GG_LUA, offset=(50, 50), interval=2):
                    continue
                if self.appear(button=BUTTON_GG_SCRIPT_START_PROCESS, offset=(50, 50), interval=2):
                    break
