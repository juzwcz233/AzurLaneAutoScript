    ...
    def _enter_gg(self):
        ...
        while 1:
            ...
            if self.appear_then_click(button=BUTTON_GG_START, offset=(50, 50)):
                continue
            if self.appear(button=MAIN_GOTO_BUILD, offset=(50, 50)):
                self.device.click(BUTTON_GG_ENTER_POS)
                continue
            if self.appear(button=REWARD_GOTO_MAIN, offset=(50, 50)):
                self.device.click(BUTTON_GG_ENTER_POS)
                continue
            if self.appear(button=DORM_CHECK, offset=(50, 50)):
                self.device.click(BUTTON_GG_ENTER_POS)
                continue
            if self.appear(button=GOTO_MAIN, offset=(50, 50)):
                self.device.click(BUTTON_GG_ENTER_POS)
                continue
